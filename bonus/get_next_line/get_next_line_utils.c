...
/*   Updated: 2022/06/21 16:34:08 by mheinze          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(char *text_data, char *buff)
{
	...
	if (!text_data || !buff)
		return (NULL);
	str = malloc(sizeof(char) * ((ft_strlen_size(text_data)
					+ ft_strlen_size(buff)) + 1));
	if (str == NULL)
		return (NULL);
	i = -1;
	j = 0;
	...
	while (buff[j] != '\0')
		str[i++] = buff[j++];
	str[ft_strlen_size(text_data) + ft_strlen_size(buff)] = '\0';
	free(text_data);
	return (str);
}
...
char	*ft_new_text(char *text_data)
{
	...
	new_text = malloc(sizeof(char) * (ft_strlen_size(text_data) - i + 1));
	if (!new_text)
		return (NULL);
	j = 0;
	i++;
